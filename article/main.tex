...
A proposta consiste em extrair, tratar e organizar esses dados utilizando técnicas de tratamento de grandes bases de dados, análise estatística e de agrupamento, possibilitando a construção de indicadores comparáveis entre escolas. A análise privilegia o cruzamento entre dois conjuntos de variáveis: 1. o desempenho dos estudantes em competências associadas ao uso e à compreensão de Tecnologias da Informação e Comunicação e 2. As características específicas da instituição de ensino em que esse participante se formou. Para o desempenho, foram consideradas competências presentes na Matriz de Referência (MR) do ENEM da área de Linguagens, Códigos e suas Tecnologias (C1 e C9) e na MR de Matemática e suas Tecnologias (C7), por estarem relacionadas a habilidades de interpretação de informações, resolução de problemas e uso de recursos tecnológicos e informacionais. Os indicadores das escolas permitem, de outro lado, caracterizar o perfil socioeconômico dos estudantes e construir índices sintéticos ou classificações que representem as instituições. O cruzamento entre desempenho nas competências selecionadas e a situação das escolas permite identificar padrões de desempenho que escapam às expectativas associadas às condições SE e, se possível, infraestruturais.

A identificação dessas unidades escolares, com desempenho resiliente, permite orientar ações de capacitação em TIC baseadas em evidências, direcionando recursos e estratégias para contextos em que tais intervenções possam produzir maior impacto. Como produto final, desenvolve-se um protótipo de mecanismo de identificação de escolas-alvo, que assume a forma de um painel analítico que permite visualizar e comparar indicadores educacionais e socioeconômicos. Espera-se que a proposta contribua tanto para o campo da ciência de dados aplicada à educação quanto para o aprimoramento de estratégias de formação em TIC no sistema educacional brasileiro.

\section{Revisão de Literatura}

O Exame Nacional do Ensino Médio (ENEM) foi criado em 1998 no Brasil com o intuito de avaliar a educação na rede de escolas do ensino médio nacionais. A adesão ao ENEM foi tímida nas primeiras edições do exame, mas ganhou força, a partir da edição de 2004, quando passou a ser o mecanismo de ingresso ao Programa Universidade para Todos (ProUni), com disputa nacional de bolsas de estudo em instituições privadas. O exame era feito em uma única tarde, composto por uma prova com 63 questões objetivas e uma redação. A sua consagração ocorre, a partir da sua edição de 2009, em que foi reestruturado para o modelo conhecido atualmente, e passou a ser também o mecanismo de ingresso ao Sistema de Seleção Unificada (Sisu). O exame passou a ser feito em 2 turnos, composto por uma redação e quatro provas de 45 questões cada uma, agrupadas em área de conhecimento, a saber: Códigos, Linguagens e suas Tecnologias, Matemática e suas Tecnologias, Ciências da Natureza e suas Tecnologias e, Ciências Humanas e suas Tecnologias. O ENEM representa, na atualidade, a principal forma de democratização do ensino superior, configurando-se também como métrica para se avaliar o Ensino Médio, a etapa final da educação básica \cite{misc:decreto_enem} e prognosticar tendências na educação no país, como base concreta para o desenvolvimento de políticas públicas.

Entre outras ferramentas relevantes para o diagnóstico e o desenvolvimento da educação brasileira temos o Censo Escolar, coordenado pelo Instituto Nacional de Estudos e Pesquisas Educacionais Anísio Teixeira (Inep), o que permite o aprofundamento ainda maior na análise sobre o ensino Brasileiro. O Censo é preenchido anualmente e com caráter declarativo obrigatório para as escolas da educação básica. A plataforma provê dados da estrutura dessas instituições que também servem de métrica para ações no país. Tanto as informações do ENEM como do Censo Escolar são disponibilizadas em seu estado bruto, através de microdados, os quais se configuram como nossas principais fontes de dados.

Entretanto, somente a disponibilidade desses dados \cite{misc:micro_enem} \cite{misc:micro_escolas} não permite um entendimento concreto sobre a educação, nem possibilita traçar uma abordagem compatível com a realidade do Brasil. Em "Influência da Renda no Desempenho dos Estudantes no Enem: uma Análise Longitudinal" \cite{article:renda_enem}, que fez uma análise de todas as edições do ENEM até 2023 cruzando os dados do questionário socioeconômico com as notas do exame, foi avaliado que, durante todos os anos, a renda familiar foi um fator decisivo para o desempenho dos alunos. Os autores também destacam a importância da ação direcionada para as escolas públicas e em situação desfavorável, pois são nessas escolas em que se encontram os estudantes mais afetados pela desigualdade social.
...
Dentre as funcionalidades implementadas foi criado um fluxo de extração dos dados em três etapas, a saber: 1. Busca-se em um cliente \textit{HTTP} o arquivo compactado das fontes, 2. Descompacta-se e salva-se o arquivo em um diretório local e 3. O documento \textit{.csv} é acessado utilizando de um caminho pré-definido pelo usuário. Após o acesso a planilha, lê-se linha à linha e as colunas e seus respectivos valores são identificados e finalmente entram na parte de filtragem.

\subsection{Pre-processamento}

Seguindo a proposta de trabalho para os resultados do ENEM; é necessário avaliar quais dos participantes não faltaram ou foram eliminados das provas de Linguagens e Códigos e Matemática (áreas correlacionadas com a capacitação em TIC).Para isso foram removidos todos os elementos em que os campos "TP\_PRESENCA\_LC" e "TP\_PRESENCA\_MT" eram diferentes de "1", o que na base indica presença.

Ao analisar os dados do Censo Escolar\cite{misc:micro_escolas} encontramos uma base com 426 campos, onde muitos desses podem causar ruídos durante o processo de classificação, então para diminuir a dimensionalidade do conjunto aplicamos a técnica de redação \textit{PCA}\cite{book:pca}. Foram incluídos somente os elementos quantitativos descritos pelo prefixo "QT" (154 campos), esses passam pelas seguintes etapas: 1. Padronização dos valores, 2. Geração da Matriz de Covariância, 3. Geração dos Autovalores e 4. Seleção dos \textit{K} campos com maiores autovalores.

\subsection{Transformação}

Para um melhor entendimento dos recortes SE das escolas foi escolhido o algoritmo \textit{K-Means++}\cite{article:kmeanspp}, nele as escolas são agrupadas de forma não supervisionada e diferente do clássico \textit{K-Means} as sementes são selecionadas de forma mais estratégica baseado em estatística. Assim é possível visualizar as características comuns e distintas das instituições e compará-las com seu desempenho no exame nacional.
\begin{comment}
\subsection{Mineração}
\end{comment}
\subsection{Interpretação}

Seguindo a proposta e metodologia deve ser desenvolvida uma interface gráfica de fácil utilização, capaz de ser executado de forma relativamente rápida em computadores de uso geral e exibir os dados obtidos pela mineração. Assim foi escolhida a biblioteca \textit{Slint} \cite{misc:slint_lib}, uma ferramenta pouco intrusiva para códigos \textit{Rust} que atende às demandas do projeto, com ela foram adicionados: comentários, um botão de execução do processo, uma barra de progresso para acompanhamento e uma tabela que exibe os resultados obtidos. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.3\textwidth]{images/ui_print1.png}
\includegraphics[width=.3\textwidth]{images/ui_print2.png}
\caption{Interface do projeto desenvolvida em \textit{Rust} \cite{misc:rust_lang} com \textit{Slint} \cite{misc:slint_lib}}
\label{fig:project_ui}
\end{figure}

\section{Implementações e Testes}

Um dos requisitos propostos pelo trabalho é a criação de uma ferramenta capaz de ser utiliza e modificada para uso pessoal e em futuras pesquisas, portanto as prioridades ao desenvolver o projeto foram performance e manutenibilidade. A escolha da linguagem \textit{Rust} \cite{misc:rust_lang} se provou capaz de atender esses requisitos e com códigos assíncronos e paralelos utilizando da biblioteca \textit{Tokio} \cite{misc:tokio_lib} houve melhoria nos resultados. Em testes feitos com um computadores de uso geral (processador \textit{Intel Core i3}, 16GB de memória RAM e GPU integrada) o fluxo completo de processamento das bases de dados do ENEM \cite{misc:micro_enem} e censo escolar \cite{misc:micro_escolas} próximos de 4 milhões e 200 mil linhas respectivamente tiveram duração inferior à 5min.

\section{Resultados Preliminares}

Com o algoritmo PCA \cite{book:pca} conseguimos obter do censo escolar \cite{misc:micro_escolas} os $K = 10$ campos quantitativos mais influentes da base de dados, a partir dos quais podemos traçar os indicadores socioeconômicos de modo mais fácil sem comprometer a acurácia, ao contrário do que ocorreria se utilizássemos todos os 154 campos.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|c|c|}
        ...
        Campo & Score \\
        \hline
        QT\_SALAS\_UTILIZADAS\_DENTRO & $28.2$ \\
        \hline
        QT\_SALAS\_UTILIZADAS\_FORA & $12.4$ \\
        \hline
        QT\_SALAS\_UTILIZADAS & $10.7$ \\
        \hline
        QT\_SALAS\_UTILIZA\_CLIMATIZADAS & $7.3$ \\
        \hline
        QT\_SALAS\_UTILIZADAS\_ACESSIVEIS & $6.9$ \\
        \hline
        QT\_EQUIP\_DVD & $5.1$ \\
        \hline
        QT\_EQUIP\_SOM & $4.6$ \\
        ...
        QT\_EQUIP\_TV & $3.8$ \\
        \hline
        QT\_EQUIP\_LOUSA\_DIGITAL & $3.5$ \\
        \hline
        QT\_EQUIP\_MULTIMIDIA & $3.3$ \\
        \hline
    \end{tabular}
    \caption{Resultado dos $K = 10$ campos quantitativos mais influentes do censo escolar \cite{misc:micro_escolas} utilizando PCA \cite{book:pca}}
    \label{tab:resultado_pca}
\end{table}
...
\end{mermaid}
\caption{Cronograma de \textit{Sprints} baseado na metodologia \textit{Scrum} (19/08/2026, sujeito a alterações)}
\label{fig:sprints}
\end{figure}

Para cada novo início de \textit{sprint} é avaliado o que foi desenvolvido na etapa anterior e criado um novo \textit{backlog} (lista de tarefas) a ser executado durante o período estimado. Atualmente (18/08/2026) o projeto se encontra na **Sprint de Pre-processamento** com as seguintes tarefas sendo desenvolvidas:
...
kanban
    Fazer
		  [Associar escolas com notas do ENEM]
		  [Identificar escolas outliers]
    Fazendo
    ...
    Feito
		  ...
		  [Finalizar PCA]
\end{mermaid}
\caption{Tabela baseada na metodologia \textit{Kanban} (18/08/2026, sujeito a alterações)}
\label{fig:kanban}
\end{figure}
