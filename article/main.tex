...
A dissertação \textit{Análise dos microdados do Enade: proposta de uma ferramenta de exploração utilizando mineração de dados} \cite{thesis:md_enade} utiliza da base de dados do Enade e metodologias de mineração e \textit{machine learning} para construir uma ferramenta \textit{web} de análise. Entre os achados da mineração foi possível gerar regras que representam condições de alunos dentro do ensino superior, o que pode ser uma forma de identifica públicos alvo. No geral a metodologia da dissertação pode ser utilizada como referencia para atingir resultados com mais qualidade.
