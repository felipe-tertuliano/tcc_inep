...
1. O que é o ENEM?
2. O que é o Censo Escolar?
3. Quais as dificuldades na educação no país
4. TIC
5. Proposta
...
O Exame Nacional do Ensino Médio (ENEM) foi criado em 1998 no Brasil com o intuito de avaliar a educação na rede de escolas do ensino médio nacionais e depois (em 2009), junto do Sistema de Seleção Unificada (Sisu) e ao Programa Universidade para Todos (ProUni), adaptado como mecanismo de acesso à educação superior. Ainda hoje o ENEM é uma das principais formas de democratização do ensino e métrica para se avaliar tendencias na educação no país, permitindo base concreta para o desenvolvimento de politicas publicas.

Dentre outras ferramentas no país o Censo Escolar, coordenado pelo Instituto Nacional de Estudos e Pesquisas Educacionais Anísio Teixeira (Inep), permite se aprofundar ainda mais na analise sobre ensino Brasileiro. Sendo feito anualmente e com caráter declarativo obrigatório para as escolas da educação básica o censo provê dados da estrutura dessas instituições que também servem de métrica para ações no país.

Entretanto, somente a disponibilidade desses dados \cite{misc:micro_enem} \cite{misc:micro_escolas} não permite um entendimento concreto sobre a educação, nem muito traçar uma abordagem compatível com a realidade do Brasil. Em "Influência Da Renda No Desempenho Dos Estudantes No Enem: Uma Análise Longitudinal" \cite{article:renda_enem}, que fez uma análise de todas as edições do ENEM até 2023 cruzando os dados do questionário socioeconômico com as notas do exame, foi avaliado que durante todos os anos a renda familiar foi um fator decisivo para a nota dos alunos. Nele também é constatado da importância da ação direcionada para as escolas publicas e em situação desfavorável, pois são nessas escolas em que se encontram os estudantes mais afetados pela desigualdade social.

Especificamente quando se discute áreas do conhecimento disparidades mais complexas podem aparecer, como em "\textit{The Analysis of Mathematics Performance in the National High School Exam in Brazil}" \cite{article:mat_enem}, onde se analisou a desempenho dos candidatos nos teste de matemática do ENEM 2022 com recortes geográficos e de gênero. Nele se mostrou que mulheres, em todos os casos, tinham piores resultados nessa área do exame, o que pode significar uma deficiência no nosso sistema no que se diz sobre de ensino de exatas.

Assim a proposta do desenvolvimento de uma ferramenta capaz de mapear esses fatores e identificar pontos de atenção se mostra uma solução possível mas também necessária. Na dissertação "Análise dos microdados do Enade: proposta de uma ferramenta de exploração utilizando mineração de dados" \cite{mastersthesis:md_enade} utilizando metodologias de mineração e \textit{machine learning} foi construída uma ferramenta \textit{web} de análise. Entre os achados à partir dos dados foi possível gerar regras que representam condições de alunos dentro do ensino superior, o que é uma forte ferramenta para intervenções direcionadas.

\begin{comment}

Para um melhor entendimento do problema e analise de possíveis soluções foram analisados alguns trabalhos correlatos. 
...
\textit{Understanding a Profile of the Participants of the Exame Nacional do Ensino Médio (ENEM), Brazil, in the Year 2019, Through Data Analysis} \cite{article:p_da_enem} analisa dentro da base de dados dos participantes do ENEM 2019 fatores que podem influenciar os resultados na prova. Utilizando de algoritmos de analise de dados foi possível traçar relação direta entre condições socioeconômicas e as notas, igualmente para grupo étnico. Mesmo que para os dados do ENEM 2024 não seja possível mais fazer essa relação ainda o estudo é uma base para formas de tratar os dados e direcionar a perspectiva da análise.
...
Para o objetivo do projeto foram escolhidas duas base de dados a serem tratadas: microdados do ENEM 2024 \cite{misc:micro_enem} e microdados do censo escolar 2024 \cite{misc:micro_escolas}. A partir desas bases foi iniciada a produção de uma ferramenta na linguagem de programação \textit{Rust} \cite{misc:rust_lang} que seja capaz de ler os microdados, tratá-los e processá-los, gerando em fim indicadores relevantes para a proposta. Um repositório \cite{misc:proj_repo} no \textit{GitHub} \cite{misc:github_repo} foi criado para o versionamento e escolhida a \textit{IDE VSCode} \cite{misc:vscode_ide} para o desenvolvimento.

\subsection{Extração dos dados}

Para extrair os dados de ambas as bases \cite{misc:micro_enem} \cite{misc:micro_escolas} foi criado uma característica chamada \textit{Data Source} capaz de acessar a fonte na \textit{internet}, baixar, descompactar e salvar essas localmente. Junto disso a \textit{Data Source} seria capaz de percorrer o arquivo e obter os elementos a serem processados pelo programa.
